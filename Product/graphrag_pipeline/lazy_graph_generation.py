...
from graphrag_pipeline.extractors.lazy_concept_extractor import LazyConceptExtractor
from graphrag_pipeline.resolvers import EntityResolver
from graphrag_pipeline.writers import GraphRAGNeo4jWriter
from graphrag_pipeline.analyzers import CommunityDetector
...
class LazyGraphRAGPipeline:
    def __init__(self, config: PipelineConfig) -> None:
        ...
        logger.info("LLM (used for Chunking if needed): %s", self.llm)
        ...
        self.extractor = LazyConceptExtractor(
    ...
    async def run(self, document_text: str, doc_title: str = "", doc_source: str = "") -> None:
        logger.info("=" * 60)
        logger.info("Starting LazyGraphRAG Pipeline")
        logger.info("=" * 60)
        ...
        logger.info("Step 1/7: Chunking (%s strategy)...", self.config.chunking.strategy)
        t0 = time.time()
        ...
        self.step_times["2. Write Chunks"] = time.time() - t0

        logger.info("Step 3/7: Extracting concepts & relations (NLP)...")
        t0 = time.time()
        extraction_results = await self.extractor.extract_from_chunks(chunks)
        ...
        logger.info("  → %d concepts, %d co-occurrences extracted.", total_entities, total_relations)

        logger.info("Step 4/7: Writing concepts & relations to Neo4j...")
        t0 = time.time()
        self.writer.write_entities_and_relations(chunk_ids, extraction_results)
        self.step_times["4. Write Concepts/Rels"] = time.time() - t0

        if self.config.entity_resolution.enabled:
            logger.info("Step 5/7: Entity Resolution (KNN → WCC → Word Distance → NO LLM)...")
            t0 = time.time()
            ...
            logger.info("Step 5/7: Entity resolution DISABLED — skipping.")
            self.step_times["5. Entity Resolution"] = 0.0

        logger.info("Step 5b/7: Embedding concepts locally...")
        t0 = time.time()
        ...
        logger.info("  → Embedding %d concepts...", len(entity_rows))
        if entity_rows:
            embed_texts = [
        ...
        logger.info("Step 6/7: Community detection (%s via Neo4j GDS)...", algo)
        t0 = time.time()
        ...
        logger.info("Step 7/7: Writing communities to Neo4j...")
        t0 = time.time()
        self.writer.write_communities(communities)
        self.step_times["7. Write Communities"] = time.time() - t0

        logger.info("Note: Skipping Step 8 (Community Summarization) for LazyGraphRAG.")
        
        total_time = time.time() - total_start_time

        logger.info("=" * 60)
        logger.info("LazyGraphRAG Pipeline COMPLETE")
        logger.info("=" * 60)
    ...
    def _print_pipeline_summary(self, total_time: float) -> None:
        ...
        print("\n" + "=" * 60)
        print("LAZY PIPELINE EXECUTION SUMMARY")
        print("=" * 60)
        ...
        print("\n--- LLM Usage (Should be 0 for extraction) ---")
...
async def async_main(document_paths: list[str]) -> None:
    config = load_config()
    pipeline = LazyGraphRAGPipeline(config)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="LazyGraphRAG Pipeline")
