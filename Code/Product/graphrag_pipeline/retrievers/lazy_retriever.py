...
class LazyRetriever:
    ...
    async def search(self, query: str) -> str:
        logger.info("[LazyRetriever] Starting lazy query processing.")
        
        logger.info("[LazyRetriever] Refining query...")
        subqueries = await self._refine_query(query)
        if not subqueries:
            subqueries = [query]
        logger.info("[LazyRetriever] Generated %d subqueries: %s", len(subqueries), subqueries)
        print("\n--- Refined Subqueries ---")
        ...
        for sq in subqueries[:self.max_subqueries]:
            logger.info("[LazyRetriever] Processing subquery: '%s'", sq)
            
            qemb = (await self.embedding_fn([sq]))[0]
            seed_entities = self._find_similar_entities(qemb)
            
            if not seed_entities:
                logger.info("[LazyRetriever]   No matching concepts found for subquery.")
                continue
                
            logger.info("[LazyRetriever]   Found %d concepts. Fetching chunks...", len(seed_entities))
            chunks = self._fetch_chunks_for_entities(seed_entities)
            
            if not chunks:
                logger.info("[LazyRetriever]   No chunks associated with these concepts.")
                continue
                
            logger.info("[LazyRetriever]   Assessing relevance of %d chunks...", len(chunks))
            ...
            valid_claims = [r for r in results if r.strip()]
            all_claims.extend(valid_claims)
            logger.info("[LazyRetriever]   Extracted claims from %d relevant chunk(s).", len(valid_claims))

        if not all_claims:
            logger.warning("[LazyRetriever] No relevant claims extracted for any subquery.")
            return "I couldn't find enough relevant information to answer your query."

        logger.info("[LazyRetriever] Synthesizing final answer from %d claim sets...", len(all_claims))
        claims_text = "\n\n".join([f"--- Claims ---\n{c}" for c in all_claims])
        reduce_prompt = REDUCE_PROMPT.format(query=query, claims=claims_text)
        
        final_answer = await self.llm.ainvoke(reduce_prompt)
        logger.info("[LazyRetriever] Final answer generated (%d chars).", len(final_answer))
    ...
    async def _refine_query(self, query: str) -> list[str]:
        ...
        try:
            ...
            logger.warning("Failed to parse subqueries JSON: %s. Raw response: %s", e, response)
