...
class BaseImporter(GraphDBBase):
    ...
    def get_batches(parameters_iterator, batch_size):
        # to not load everything into memory directly and crash it, here we take it in batches.
        while True:
...
#                  transaction_batch_store                                   aggregate_batch_store
# Execution        Runs the query $N$ times per batch.                       Runs the query once per batch.
# Speed            Moderate.                                                 Very High.
# Query Style      Standard parameters: {name: "Alice"}.                     Expects a list: {"batch": [...]}.
# Memory           Low.                                                      Slightly higher (holds the batch list).